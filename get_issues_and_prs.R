...
# Set up pin boards
meta_board <- pins::board_folder('/srv/docker-pins/github/meta')
repo_board <- pins::board_folder('/srv/docker-pins/github/repos')

# Load the full repo list
meta_repos <- pins::pin_read(meta_board, 'repos')
...
safe_pin_meta <- purrr::possibly(~{pin_meta(.x, .y)$created |> as.character()},
                                 otherwise = NA)
meta_repos |>
  dplyr::mutate(pin_name = glue::glue("{org}|{repo}")) |>
  dplyr::mutate(pin_time = purrr::map_chr(pin_name, ~safe_pin_meta(repo_board, .x))) |>
  arrange(pin_time) |>
